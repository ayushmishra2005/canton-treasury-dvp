...
"$(agave_devnet_validator)" --reset --quiet --ledger "$ledger_dir" --rpc-port 8899 --faucet-port 9900 \
  ...
  --bpf-program "$ESCROW_PROGRAM_ID" "$(escrow_so)" >"$tmp_dir/ctd-solana.log" 2>&1 &
started_pids+=("$!")
...
cd zama
npx hardhat node --hostname 127.0.0.1 --port 8545 >"$tmp_dir/ctd-hardhat.log" 2>&1 &
started_pids+=("$!")
cd "$repo_root"
for _ in $(seq 1 60); do
...
(cd zama && ZAMA_CAPACITY=200000000000 npx hardhat run scripts/deploy.ts --network localhost | tee "$tmp_dir/ctd-zama-deploy.log")
...
python3 - <<'PY' > "$tmp_dir/ctd-live-isolation-input.json"
...
dpm script --dar daml/bridge-tests/.daml/dist/canton-treasury-dvp-bridge-tests-0.1.0.dar \
  ...
  --input-file "$tmp_dir/ctd-live-isolation-input.json" \
  --wall-clock-time > "$tmp_dir/ctd-live-prepare.log" 2>&1 \
  || { tail -40 "$tmp_dir/ctd-live-prepare.log" >&2; fail "live isolation prepare failed"; }
...
dpm script --dar daml/bridge-tests/.daml/dist/canton-treasury-dvp-bridge-tests-0.1.0.dar \
  ...
  --participant-config "$run_dir/participants.json" \
  --input-file "$tmp_dir/ctd-live-isolation-input.json" \
  --wall-clock-time > "$tmp_dir/ctd-live-isolation.log" 2>&1 \
  || { tail -40 "$tmp_dir/ctd-live-isolation.log" >&2; fail "live isolation of two identical-term operations failed"; }
grep -q LIVE_ISOLATION_OK "$tmp_dir/ctd-live-isolation.log" \
  || fail "live isolation did not print LIVE_ISOLATION_OK"
grep -q 'LIVE_ISOLATION_BROAD_LOOKUP 2' "$tmp_dir/ctd-live-isolation.log" \
  || fail "live isolation did not prove the broad party-and-amount lookup"
daml_marker() {
  local file="$1" key="$2"
  sed -n "s/.*${key} \\([0-9a-fA-F]*\\).*/\\1/p" "$file" | tail -1
}
iso_a="$(daml_marker "$tmp_dir/ctd-live-isolation.log" LIVE_ISOLATION_BINDING_A)"
iso_b="$(daml_marker "$tmp_dir/ctd-live-isolation.log" LIVE_ISOLATION_BINDING_B)"
[[ -n "$iso_a" && -n "$iso_b" && "$iso_a" != "$iso_b" ]] \
  || fail "live isolation did not bind A and B to different trades: $iso_a $iso_b"
iso_mint_a="$(daml_marker "$tmp_dir/ctd-live-isolation.log" LIVE_ISOLATION_MINT_A)"
iso_mint_b="$(daml_marker "$tmp_dir/ctd-live-isolation.log" LIVE_ISOLATION_MINT_B)"
...
used_holds="$(sed -n 's/.*CANTON_USED_TREASURY_HOLDING \([0-9a-fA-F]*\).*/\1/p' "$tmp_dir/ctd-live-isolation.log" | sort -u)"
...
run_workflow() {
  local extra=("$@")
  set +e
  env "${workflow_env[@]}" cargo run --manifest-path bridge/Cargo.toml --quiet -- workflow "${extra[@]}" | tee -a "$tmp_dir/ctd-workflow.log"
  local status=${PIPESTATUS[0]}
  set -e
  if [[ "$status" -ne 0 ]]; then
    fail "bridge workflow failed at ${extra[*]:-complete}; last sizes: $(grep TX_SIZE "$tmp_dir/ctd-workflow.log" || true)"
  fi
}
...
grep -q EXPIRY_RECOVERY_MINT_REJECTED "$tmp_dir/ctd-workflow.log" \
  || fail "mint approval was not rejected after the original deadline"
grep -q EXPIRY_RECOVERY_CANCEL_CONFIRMED "$tmp_dir/ctd-workflow.log" \
  || fail "expiry recovery did not cancel through Relayer"
grep -q EXPIRY_RECOVERY_COMPLETE "$tmp_dir/ctd-workflow.log" \
...
grep -q "RECOVERY_RESULT cancelled" "$tmp_dir/ctd-workflow.log" \
  || fail "expiry recovery did not record a cancelled recovery result"
grep -q "RECOVERY_DURATION_CHAIN_SECS" "$tmp_dir/ctd-workflow.log" \
...
last_marker() {
  grep "$1" "$tmp_dir/ctd-workflow.log" | tail -1
}
...
[[ "$(grep -c ZAMA_RESERVATION_REJECTED "$tmp_dir/ctd-workflow.log")" -ge 2 ]] \
  || fail "rejected reservation was not re-checked on retry"
assert_no_lock_or_mint "$reject_dir"
if grep -q RELAYER_RELEASE_CONFIRMED "$tmp_dir/ctd-workflow.log"; then
...
zama_rpc finalize "$reject_reservation" >"$tmp_dir/ctd-zama-finalize.log"
grep -q ZAMA_RESULT "$tmp_dir/ctd-zama-finalize.log" || fail "finalize of the rejected reservation did not return"
...
[[ "$(grep -c ZAMA_RESERVATION_REJECTED "$tmp_dir/ctd-workflow.log")" -ge 3 ]] \
...
zama_rpc cancel "$live_probe_id" >"$tmp_dir/ctd-zama-cancel-live-probe.log"
...
grep -q "RECOVERY_WAIT_UNBOUNDED operator_or_quorum" "$tmp_dir/ctd-workflow.log" \
  || fail "attester disagreement did not record that quorum wait is unbounded"
grep -q CHAIN_CLOCK "$tmp_dir/ctd-workflow.log" \
...
grep -q DVP_BUYER_TREASURY "$tmp_dir/ctd-workflow.log" \
...
grep -q "LOCKED_STATE solana_vault" "$tmp_dir/ctd-workflow.log" \
...
grep -q "FAULT_INJECTED expiry_after_settlement" "$tmp_dir/ctd-workflow.log" \
...
assert_journal_step "$journal_dir" released
grep -q ZAMA_REDEEM_OK "$tmp_dir/ctd-workflow.log" || fail "Zama redemption did not land before the journal save"
...
tx_size_before_complete="$(grep -c TX_SIZE "$tmp_dir/ctd-workflow.log" || true)"
...
grep -q OPERATION_RECORDED_COMPLETE "$tmp_dir/ctd-workflow.log" \
  || fail "resume did not record the completed operation after Zama redemption"
grep -q COMPLETED_RESUME_SKIP_SETUP "$tmp_dir/ctd-workflow.log" \
  || fail "completed resume did not skip setup and funding"
grep -q CANTON_VERIFY_OK "$tmp_dir/ctd-workflow.log" \
...
[[ "$(grep -c TX_SIZE "$tmp_dir/ctd-workflow.log" || true)" == "$tx_size_before_complete" ]] \
...
env CANTON_RUN_DIR="$run_dir" CANTON_JAR="$canton_jar" \
  ...
  java -jar "$canton_jar" run canton/scripts/verify-bridge-completion.canton \
    -c canton/remote-console.conf --no-tty --log-level-stdout WARN \
    > "$tmp_dir/ctd-canton-other-lock.log" 2>&1
other_lock_status=$?
set -e
[[ "$other_lock_status" -ne 0 ]] || fail "ledger verify accepted another operation lock"
if ! grep -qE 'CANTON_VERIFY_FAIL|CANTON_HISTORY_OTHER_LOCK|another operation' "$tmp_dir/ctd-canton-other-lock.log"; then
  fail "other-operation verify did not fail closed: $(tail -20 "$tmp_dir/ctd-canton-other-lock.log")"
fi
set +e
env CANTON_RUN_DIR="$run_dir" CANTON_JAR="$canton_jar" \
  ...
  java -jar "$canton_jar" run canton/scripts/does-not-exist.canton \
    -c canton/remote-console.conf --no-tty --log-level-stdout WARN \
    > "$tmp_dir/ctd-canton-unreadable.log" 2>&1
unreadable_status=$?
set -e
[[ "$unreadable_status" -ne 0 ]] || fail "unreadable ledger verify was treated as success"
if grep -q CANTON_VERIFY_OK "$tmp_dir/ctd-canton-unreadable.log"; then
  fail "unreadable ledger verify printed success"
fi
python3 - "$journal_dir/journal.json" <<'PY' > "$tmp_dir/ctd-canton-missing-input.json"
import json, sys
journal = json.load(open(sys.argv[1]))
print(json.dumps({
    "lockId": "missing-lock",
...
dpm script --dar daml/bridge-tests/.daml/dist/canton-treasury-dvp-bridge-tests-0.1.0.dar \
  ...
  --participant-config "$run_dir/participants.json" \
  --input-file "$tmp_dir/ctd-canton-missing-input.json" \
  --wall-clock-time > "$tmp_dir/ctd-canton-missing.log" 2>&1
missing_status=$?
set -e
[[ "$missing_status" -ne 0 ]] || fail "missing ledger evidence was treated as completion"
if grep -q CANTON_ACS_OK "$tmp_dir/ctd-canton-missing.log"; then
  fail "missing lock printed ACS success"
fi

echo "BRIDGE_RESUME_COMPLETED_AGAIN"
tx_size_before_second="$(grep -c TX_SIZE "$tmp_dir/ctd-workflow.log" || true)"
zama_before_second="$(zama_block)"
BRIDGE_MINT_EXPIRY_SECS=90 BRIDGE_JOURNAL_DIR="$journal_dir" \
  run_workflow --journal "$journal_dir" --resume --stop-after zama_redeemed
assert_journal_step "$journal_dir" zama_redeemed
[[ "$(grep -c OPERATION_RECORDED_COMPLETE "$tmp_dir/ctd-workflow.log")" -ge 2 ]] \
  || fail "second resume of a completed operation did not succeed"
[[ "$(grep -c COMPLETED_RESUME_SKIP_SETUP "$tmp_dir/ctd-workflow.log")" -ge 2 ]] \
  || fail "second completed resume did not skip setup and funding"
[[ "$(grep -c CANTON_VERIFY_OK "$tmp_dir/ctd-workflow.log")" -ge 2 ]] \
  || fail "second completed resume did not verify Canton from ledger history"
[[ "$(grep -c TX_SIZE "$tmp_dir/ctd-workflow.log" || true)" == "$tx_size_before_second" ]] \
...
grep -q CANTON_MINT_HOLDING "$tmp_dir/ctd-workflow.log" || fail "Canton mint holding was not recorded"
...
grep -q DVP_PAYMENT_AMOUNT "$tmp_dir/ctd-workflow.log" || fail "DvP payment amount was not asserted"
grep -q CANTON_REDEEM "$tmp_dir/ctd-workflow.log" || fail "seller redemption was not recorded"
grep -q RELAYER_RELEASE_CONFIRMED "$tmp_dir/ctd-workflow.log" || fail "confidential release through Relayer did not confirm"
grep -q ZAMA_REDEEM_OK "$tmp_dir/ctd-workflow.log" || fail "Zama redemption did not succeed"
grep -q EXPIRY_RECOVERY_COMPLETE "$tmp_dir/ctd-workflow.log" || fail "expiry recovery evidence is missing"

python3 - "$journal_dir/journal.json" "$tmp_dir/ctd-workflow.log" <<'PY'
import json, re, sys
journal = json.load(open(sys.argv[1]))
...
zama_rpc cancel "$after_probe_id" >"$tmp_dir/ctd-zama-cancel-after-probe.log"
zama_rpc cancel "$leftover_probe_id" >"$tmp_dir/ctd-zama-cancel-leftover-probe.log"
[[ "$(zama_result_line approved "$unrelated_id")" == *'"approved":true'* ]] \
